pro co_add_ccfs,orders,dataset,modelname,ccv=ccv
  ...
  if n_elements(orders) ne n_orders then begin
     print,"Orders don't match the size of the ccf stack. Stopping."
     stop
     return
  endif
...
;sum in the template...
  ...
  cgps_open,inpath+'/Order_weights.ps'
  cgplot,orders,order_weights,xtitle='Order number',ytitle='Weight',charsize=0.9
  cgps_close
