...
def view_model(modelname,library,noshow=False):
    ...
    wlm,fxm=get_model(modelname,library=library)
    plt.plot(wlm,fxm)
    if noshow != True:
        plt.show()

def model_from_pyeta(modelpath):
    """This code converts a py-eta model transmission spectrum (likely generated by
    Lorenzo Pino) to a format compatible with the model library of this suite.
    All that is needed is the path to the pyeta file. A FITS file with the same name
    is placed in the same location. You will then need to add this to a model library
    in order to access it as a cross-correlation template/model.

    The program assumes that eta has written out wavelengths in Angstrom and exoplanet
    transit radii (i.e. Rp^2/Rs^2). This may not always be true. Adjust accordingly."""
    import matplotlib.pyplot as plt
    import pdb
    import lib.utils as ut
    import numpy as np
    import os.path
    ut.typetest('modelpath in model_from_pyeta',modelpath,str)
    with open(modelpath,'r') as f:
        lines = f.read().splitlines()
    wl = []
    Rbar = []
    inpath, filename = os.path.split(modelpath)
    modelname = filename.split('.')[0]
    outpath = inpath+'/'+modelname+'.fits'
    for l in lines:
        fields = l.split('\t')
        if len(fields) == 3 and fields[0] != '#':
            wl.append(float(fields[0])/10.0)#Angstrom to nm.
            Rbar.append(1.0-float(fields[1]))
    N = len(wl)
    out = np.zeros((2,N))
    out[0] = wl
    out[1] = Rbar
    print('Model written to '+outpath)
    ut.writefits(outpath,out)
